...
/datum/multitool_menu_host/ui_data(mob/user)
	...
	data["isAttachedAlreadyInBuffer"] = attached_menu?.holder == multitool.buffer
...
/datum/multitool_menu
	...
	/// The state of the holder can be changed only while this is not null.
	var/obj/item/multitool/multitool
...
/datum/multitool_menu/proc/powered()
	if(!ismachinery(holder))
		return TRUE
	var/obj/machinery/my_holder = holder
	return my_holder.powered(my_holder.power_channel)
...
/datum/multitool_menu/idtag/freq/air_sensor/set_tag(new_tag)
	...
	if(!is_the_tag_unique(new_tag, my_holder.frequency))
		my_holder.visible_message("<span class='warning'>There is already the same ID tag on the frequency.</span>")
		return
	my_holder.id_tag = new_tag
...
////////////////////////////////
//	Multitool menu "general_air_control"
//  This does not use the id_tag stuff, only the frequency stuff.
////////////////////////////////
...
////////////////////////////////
//	Multitool menu "large_tank_control"
////////////////////////////////
	...
	switch(action)
		if("link_input")
			if(notify_if_the_buffer_is_not_ok())
				return FALSE
			var/obj/machinery/computer/general_air_control/large_tank_control/my_holder = holder
			my_holder.link_input(multitool.buffer)
		if("link_output")
			if(notify_if_the_buffer_is_not_ok())
				return FALSE
			var/obj/machinery/computer/general_air_control/large_tank_control/my_holder = holder
			my_holder.link_output(multitool.buffer)
		if("unlink_input")
			var/obj/machinery/computer/general_air_control/large_tank_control/my_holder = holder
...
/datum/multitool_menu/idtag/freq/general_air_control/large_tank_control/proc/notify_if_the_buffer_is_not_ok()
	if(is_null_idtag())
		holder.visible_message("<span class='warning'>The ID tag of the device must be not null.</span>")
		return TRUE
	if(is_idtag_already_linked())
		holder.visible_message("<span class='warning'>The device with the same ID tag is already connected to [holder].</span>")
		return TRUE
	return FALSE
